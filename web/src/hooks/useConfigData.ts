...
export function useConfigData() {
  ...
  // 配置更新方法
  const updateFullConfig = useCallback(async (newConfig: AppConfig) => {
    try {
  ...
  // MCP 相关更新方法
  const updateMcpEndpointData = useCallback(async (endpoint: string | string[]) => {
    try {
  ...
  const updateMcpServersData = useCallback(async (servers: Record<string, MCPServerConfig>) => {
    try {
  ...
  // 系统配置更新方法
  const updateConnectionConfigData = useCallback(async (connection: ConnectionConfig) => {
    try {
  ...
  const updateModelScopeConfigData = useCallback(async (modelscope: ModelScopeConfig) => {
    try {
  ...
  const updateWebUIConfigData = useCallback(async (webUI: WebUIConfig) => {
    try {
