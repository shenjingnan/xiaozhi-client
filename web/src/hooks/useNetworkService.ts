...
import { networkService, ConnectionState } from "../services";
...
export function useNetworkService() {
   ...
   * 重启服务并等待完成通知
   */
  const restartServiceWithNotification = useCallback(async (timeout = 30000): Promise<void> => {
    try {
   ...
   * 设置自定义 WebSocket URL
   */
  const setCustomWsUrl = useCallback((url: string): void => {
   ...
   * 端口切换功能 (保留向后兼容)
   */
  const changePort = useCallback(async (newPort: number): Promise<void> => {
    try {
