...
function WebSocketSyncTest() {
  ...
  const runSyncTest = () => {
    ...
    if (hookConnected === storeConnectedValue) {
      ...
      addTestResult(
        `❌ 连接状态不同步: Hook=${hookConnected}, Store=${storeConnectedValue}`
      );
    }
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <div className="bg-white dark:bg-gray-800 p-3 rounded text-xs space-y-1">
            ...
            >
              连接状态: Hook={String(webSocketState.connected)} | Store=
              {String(storeConnected)}
            </div>
            ...
            >
              状态对象: Hook={webSocketState.status?.status || "null"} | Store=
            ...
            >
              WebSocket URL:{" "}
              {webSocketState.wsUrl === fullStore.wsUrl ? "同步" : "不同步"}
            </div>
          </div>
        </div>
