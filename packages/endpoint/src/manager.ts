...
const logger = createLogger("EndpointManager");
...
export class EndpointManager extends EventEmitter {
  ...
  private addEndpointInternal(endpoint: Endpoint): void {
    ...
    if (this.endpoints.has(url)) {
      logger.debug(
        `接入点 ${sliceEndpoint(url)} 已存在，跳过添加`
      );
      return;
    }
  ...
  async removeEndpoint(endpoint: Endpoint): Promise<void> {
    ...
    if (!this.endpoints.has(url)) {
      logger.debug(
        `接入点 ${sliceEndpoint(url)} 不存在，跳过移除`
      );
      return;
    }
  ...
  async connect(endpoint?: string): Promise<void> {
    ...
    if (endpoint) {
      ...
      if (status?.connected) {
        logger.debug(
          `接入点已连接，跳过: ${sliceEndpoint(endpoint)}`
        );
        return;
      }
    ...
    // 连接所有未连接的 Endpoint
    logger.debug(
      `开始连接接入点，总数: ${this.endpoints.size}`
    );
    ...
    logger.info(
      `连接完成: 成功 ${connectedCount}/${this.endpoints.size}`
    );
  }
