...
export class EndpointManager extends EventEmitter {
  ...
  setMcpManager(mcpManager: IMCPServiceManager): void {
    ...
    if ("on" in mcpManager && typeof mcpManager.on === "function") {
      const connectedHandler = (...args: unknown[]) => {
        ...
        logger.info(
          `MCP 服务已连接: ${data.serverName}, 工具数: ${data.tools?.length || 0}`
        );
      };
      const errorHandler = (...args: unknown[]) => {
        const data = args[0] as MCPErrorEvent;
        logger.error(
          `MCP 服务连接失败: ${data.serverName}`,
          data.error
        );
      };
  ...
  private addEndpointInternal(endpoint: Endpoint): void {
    ...
    if (this.endpoints.has(url)) {
      logger.debug(`接入点 ${sliceEndpoint(url)} 已存在，跳过添加`);
      return;
    }

    logger.debug(`添加接入点: ${sliceEndpoint(url)}`);
  ...
  async removeEndpoint(endpoint: Endpoint): Promise<void> {
    ...
    if (!this.endpoints.has(url)) {
      logger.debug(`接入点 ${sliceEndpoint(url)} 不存在，跳过移除`);
      return;
    }

    logger.debug(`移除接入点: ${sliceEndpoint(url)}`);
  ...
  async connect(endpoint?: string): Promise<void> {
    ...
    if (endpoint) {
      ...
      if (status?.connected) {
        logger.debug(`接入点已连接，跳过: ${sliceEndpoint(endpoint)}`);
        return;
      }
    ...
    // 连接所有未连接的 Endpoint
    logger.debug(`开始连接接入点，总数: ${this.endpoints.size}`);
    ...
    for (const [url, endpoint] of this.endpoints) {
      ...
      // 跳过已连接的端点
      if (status?.connected) {
        logger.debug(`接入点已连接，跳过: ${sliceEndpoint(url)}`);
        continue;
      }

      promises.push(
        this.connectSingleEndpoint(url, endpoint).catch((error) => {
          logger.error(`连接失败: ${sliceEndpoint(url)}`, error);
          // 更新失败状态
          const status = this.connectionStates.get(url);
          if (status) {
    ...
    logger.info(`连接完成: 成功 ${connectedCount}/${this.endpoints.size}`);
  }
  ...
  async disconnect(endpoint?: string): Promise<void> {
    ...
    if (endpoint) {
      ...
      logger.debug(`接入点已断开: ${sliceEndpoint(endpoint)}`);
      return;
    }

    // 断开所有连接
    logger.debug("开始断开所有连接");
    ...
    for (const [url, endpoint] of this.endpoints) {
      promises.push(
        this.reconnectSingleEndpoint(url, endpoint).catch((error) => {
          logger.error(`重连失败: ${sliceEndpoint(url)}`, error);
        })
      );
    }
  ...
  private async connectSingleEndpoint(
    ...
    logger.debug(`连接端点: ${sliceEndpoint(url)}`);
    ...
    logger.info(`端点连接成功: ${sliceEndpoint(url)}`);
  }
  ...
  private async reconnectSingleEndpoint(
    ...
    logger.debug(`重连端点: ${sliceEndpoint(url)}`);
    ...
    logger.info(`端点重连成功: ${sliceEndpoint(url)}`);
  }
}
