...
export class InternalMCPManagerAdapter implements IMCPServiceManager {
  ...
  private errorHandler: (data: { serverName: string; error: unknown }) => void;
  ...
  async cleanup(): Promise<void> {
    // 移除事件监听器以防止内存泄漏
    this.mcpManager.off("connected", this.connectedHandler);
    this.mcpManager.off("error", this.errorHandler);
