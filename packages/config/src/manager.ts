...
export class ConfigManager {
  ...
  public removeMcpServer(serverName: string): void {
    ...
    // 记录清理结果
    logger.log("成功移除 MCP 服务", { serverName });
  }
        ...
        case "json5":
          ...
          try {
            if (this.json5Writer) {
              ...
              logger.warn("没有 JSON5 适配器实例，使用 comment-json 序列化");
  ...
  public async updateToolUsageStats(
    ...
    try {
      ...
      if (typeof arg2 === "string" && arg3) {
        ...
        logger.log("工具使用统计已更新", { serverName, toolName });
      } else {
        ...
        logger.log("CustomMCP 工具使用统计已更新", { toolName });
      }
    } catch (error) {
      // 错误不应该影响主要的工具调用流程
      if (typeof arg2 === "string" && arg3) {
        const serverName = arg1;
        const toolName = arg2;
        logger.error("更新工具使用统计失败", { serverName, toolName, error });
      } else {
        const toolName = arg1;
        logger.error("更新 CustomMCP 工具使用统计失败", { toolName, error });
      }
    }
  }
  ...
  public validateCustomMCPTools(tools: CustomMCPTool[]): boolean {
    ...
    for (const tool of tools) {
      ...
      if (!tool.name || typeof tool.name !== "string") {
        logger.warn("CustomMCP 工具缺少有效的 name 字段", { tool });
        return false;
      }

      if (!tool.description || typeof tool.description !== "string") {
        logger.warn("CustomMCP 工具缺少有效的 description 字段", {
          toolName: tool.name,
        });
        return false;
      }
      ...
      if (!tool.inputSchema || typeof tool.inputSchema !== "object") {
        logger.warn("CustomMCP 工具缺少有效的 inputSchema 字段", {
          toolName: tool.name,
        });
        return false;
      }
      ...
      if (!tool.handler || typeof tool.handler !== "object") {
        logger.warn("CustomMCP 工具缺少有效的 handler 字段", {
          toolName: tool.name,
        });
        return false;
      }
      ...
      if (
        ...
        logger.warn("CustomMCP 工具的 handler.type 类型无效", {
          toolName: tool.name,
  ...
  private validateHandlerConfig(
    ...
    switch (handler.type) {
      ...
      default:
        logger.warn("CustomMCP 工具使用了未知的处理器类型", {
          toolName,
  ...
  private validateProxyHandler(
    ...
    if (!handler.platform) {
      logger.warn("CustomMCP 工具的 proxy 处理器缺少 platform 字段", {
        toolName,
      });
      return false;
    }
    ...
    if (!["coze", "openai", "anthropic", "custom"].includes(handler.platform)) {
      logger.warn("CustomMCP 工具的 proxy 处理器使用了不支持的平台", {
        toolName,
    ...
    if (!handler.config || typeof handler.config !== "object") {
      logger.warn("CustomMCP 工具的 proxy 处理器缺少 config 字段", {
        toolName,
      });
      return false;
    }
  ...
  private validateHttpHandler(
    ...
    if (!handler.url || typeof handler.url !== "string") {
      logger.warn("CustomMCP 工具的 http 处理器缺少有效的 url 字段", {
        toolName,
      });
      return false;
    }
    ...
    try {
      ...
      logger.warn("CustomMCP 工具的 http 处理器 url 格式无效", {
        toolName,
        url: handler.url,
      });
      return false;
    ...
    if (
      ...
      logger.warn("CustomMCP 工具的 http 处理器使用了不支持的 HTTP 方法", {
        toolName,
  ...
  private validateFunctionHandler(
    ...
    if (!handler.module || typeof handler.module !== "string") {
      logger.warn("CustomMCP 工具的 function 处理器缺少有效的 module 字段", {
        toolName,
      });
      return false;
    }
    ...
    if (!handler.function || typeof handler.function !== "string") {
      logger.warn("CustomMCP 工具的 function 处理器缺少有效的 function 字段", {
        toolName,
      });
      return false;
    }
  ...
  private validateScriptHandler(
    ...
    if (!handler.script || typeof handler.script !== "string") {
      logger.warn("CustomMCP 工具的 script 处理器缺少有效的 script 字段", {
        toolName,
      });
      return false;
    }
    ...
    if (
      ...
      logger.warn("CustomMCP 工具的 script 处理器使用了不支持的解释器", {
        toolName,
  ...
  private validateChainHandler(
    ...
    if (
      ...
      logger.warn("CustomMCP 工具的 chain 处理器缺少有效的 tools 数组", {
        toolName,
      });
      return false;
    }
    ...
    if (!["sequential", "parallel"].includes(handler.mode)) {
      logger.warn("CustomMCP 工具的 chain 处理器使用了不支持的执行模式", {
        toolName,
        mode: handler.mode,
      });
      return false;
    ...
    if (!["stop", "continue", "retry"].includes(handler.error_handling)) {
      logger.warn("CustomMCP 工具的 chain 处理器使用了不支持的错误处理策略", {
        toolName,
  ...
  private validateMCPHandler(
    ...
    if (!handler.config || typeof handler.config !== "object") {
      logger.warn("CustomMCP 工具的 mcp 处理器缺少 config 字段", { toolName });
      return false;
    }
    ...
    if (
      ...
      logger.warn("CustomMCP 工具的 mcp 处理器缺少有效的 serviceName", {
        toolName,
      });
      return false;
    }
    ...
    if (
      ...
      logger.warn("CustomMCP 工具的 mcp 处理器缺少有效的 toolName", {
        toolName,
      });
      return false;
    }
  ...
  public addCustomMCPTool(tool: CustomMCPTool): void {
    ...
    logger.log("成功添加自定义 MCP 工具", { toolName: tool.name });
  }
  ...
  public removeCustomMCPTool(toolName: string): void {
    ...
    logger.log("成功删除自定义 MCP 工具", { toolName });
  }
  ...
  public updateCustomMCPTool(
    ...
    logger.log("成功更新自定义 MCP 工具", { toolName });
  }
  ...
  public updateCustomMCPTools(tools: CustomMCPTool[]): void {
    ...
    logger.log("成功更新自定义 MCP 工具配置", { count: tools.length });
  }
  ...
  private notifyConfigUpdate(config: AppConfig): void {
    try {
      ...
      if (webServer && typeof webServer.broadcastConfigUpdate === "function") {
        ...
        logger.log("已通过 WebSocket 广播配置更新");
      }
    } catch (error) {
      // 静默处理错误，不影响配置保存的主要功能
      logger.warn(
  ...
  private async updateCustomMCPToolStats(
    ...
    try {
      ...
      if (typeof arg3 === "string") {
        const serverName = arg1;
        const toolName = arg2;
        logger.error("更新 customMCP 工具统计信息失败", {
          serverName,
          toolName,
          error,
        });
        ...
        const toolName = arg1;
        logger.error("更新 customMCP 工具统计信息失败", { toolName, error });
      }
  ...
  private async acquireStatsUpdateLock(toolKey: string): Promise<boolean> {
    if (this.statsUpdateLocks.has(toolKey)) {
      logger.log("工具统计更新正在进行中，跳过本次更新", { toolKey });
      return false;
    }
  ...
  private releaseStatsUpdateLock(toolKey: string): void {
    ...
    logger.log("已释放工具的统计更新锁", { toolKey });
  }
  ...
  public async updateToolUsageStatsWithLock(
    ...
    try {
      ...
      logger.log("工具统计更新完成", { toolName });
    } catch (error) {
      logger.error("工具统计更新失败", { toolName, error });
      throw error;
    } finally {
      this.releaseStatsUpdateLock(toolKey);
    }
  ...
  public async updateMCPServerToolStatsWithLock(
    ...
    try {
      ...
      logger.log("MCP 服务工具统计更新完成", { serviceName, toolName });
    } catch (error) {
      logger.error("MCP 服务工具统计更新失败", {
        serviceName,
        toolName,
        error,
      });
  ...
  public clearAllStatsUpdateLocks(): void {
    ...
    if (lockCount > 0) {
      logger.log("已清理统计更新锁", { count: lockCount });
    }
  }
