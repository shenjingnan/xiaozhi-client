...
export class ASR extends EventEmitter {
  ...
  constructor(options: ASROption) {
    ...
    if (options.bytedance) {
      ...
      if (isV2Config(options.bytedance)) {
        ...
        const v2Request = (
          options.bytedance as { v2?: { request?: Record<string, unknown> } }
        )?.v2?.request;
        if (v2Request) {
          this.segDuration =
            (v2Request.segDuration as number) || this.segDuration;
          this.nbest = (v2Request.nbest as number) || this.nbest;
          this.workflow = (v2Request.workflow as string) || this.workflow;
          this.showLanguage =
            v2Request.showLanguage !== undefined
              ? (v2Request.showLanguage as boolean)
              : this.showLanguage;
          this.showUtterances =
            v2Request.showUtterances !== undefined
              ? (v2Request.showUtterances as boolean)
              : this.showUtterances;
  ...
  private async processAudioDataCommon(
    ...
    // 构建完整的客户端请求：header + payload size (4 字节) + payload
