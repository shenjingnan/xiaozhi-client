...
export abstract class ByteDanceController {
  ...
  /**
   * 子类实现此方法决定是否在注册事件处理器之前连接
   * V2: 返回 false（先注册事件，后连接）
   * V3: 返回 true（先连接，后注册事件）
   */
  protected abstract shouldConnectBeforeEventHandlers(): boolean;
  ...
  async *listen(
    ...
    // 根据子类决定连接和事件注册的顺序
    if (this.shouldConnectBeforeEventHandlers()) {
      // V3: 先连接，后注册事件
      await this.asr.connect();
      registerEventHandlers();
    } else {
      // V2: 先注册事件，后连接（避免错过初始响应）
      registerEventHandlers();
      await this.asr.connect();
    }
