...
export abstract class ByteDanceController {
  ...
  /**
   * 判断是否应该在注册事件监听器之前连接服务器
   * V2: 返回 false（先注册监听器，后连接）
   * V3: 返回 true（先连接，后注册监听器）
   */
  protected abstract shouldConnectBeforeListeners(): boolean;
  ...
  async *listen(
    ...
    // V3 在注册监听器之前连接，V2 在之后连接
    if (this.shouldConnectBeforeListeners()) {
    ...
    this.asr.on("result", (data) => {
      ...
      // 判断是否为最终结果
      // V2/V3: sequence < 0 或 utterances 中 definite=true 或 termination=true 表示最终结果
