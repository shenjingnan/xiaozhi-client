...
1. 在管理界面中，找到并点击"接入点管理"按钮
2. 在弹出的对话框中，点击"添加接入点"按钮
3. 输入小智服务端提供的接入点地址：
   - 格式：`wss://api.xiaozhi.me/mcp/?token=YOUR_TOKEN`
   - 或：`ws://your-server:port/mcp`
4. 点击"确认"保存接入点配置
5. 等待连接状态显示为"已连接"（绿色图标）

<Callout type="warning">
  **注意**：接入点地址必须以 `ws://` 或 `wss://` 开头，否则会被认为格式无效。
</Callout>

## 重启服务

配置更改后，可以通过以下两种方式重启服务：

### 方式一：通过 Web UI 重启（推荐）

1. 在管理界面中，找到并点击"重启服务"按钮
2. 确认重启操作
3. 等待服务重启完成（按钮会显示"重启中..."状态）
...
重启后，访问 http://localhost:9999 查看服务状态。
</Steps>
