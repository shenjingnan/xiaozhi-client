...
<img style={{ margin: '10px 0'}} src="/images/config-asr-doubao/1-create-app.png" alt="第一步：打开豆包语音控制台 进入 应用管理 界面" />

然后在菜单中点击 **豆包流式语音识别模型2.0**
获取对应的 `appid` 和 `accessToken`
<img style={{ margin: '10px 0'}} src="/images/config-asr-doubao/2-get-appid-and-access-token.png" alt="获取对应的 `appid` 和 `accessToken`" />
...
<img style={{ margin: '10px 0'}} src="/images/change-ota-config-url/1-select-menu-xiaozhi-assistant.png" alt="在菜单中选择 `Xiaozhi Assistant` 这个菜单项，然后按下 `Enter` 键进入配置菜单" />

然后选择 `OTA Config URL` 这个选项，然后按下 `Enter` 键进入配置菜单，输入 `http://<你的IP地址>:9999/xiaozhi/ota/`
<img style={{ margin: '10px 0'}} src="/images/change-ota-config-url/2-change-ota-url.png" alt="修改 OTA Config URL" />
