...
import type { CustomMCPTool } from "@/config";
...
import { CozeWorkflowHandler } from "./coze-workflow.handler.js";
import { ToolErrorHandler } from "./tool-error.handler.js";
import { ToolPreChecker } from "./tool-precheck.handler.js";
...
export class MCPToolHandler {
  ...
  private cozeWorkflowHandler: CozeWorkflowHandler;
  private errorHandler: ToolErrorHandler;
  private preChecker: ToolPreChecker;

  constructor() {
    this.ajv = new Ajv({ allErrors: true, verbose: true });
    this.validator = new ToolValidator();
    this.cozeWorkflowHandler = new CozeWorkflowHandler(this.validator);
    this.errorHandler = new ToolErrorHandler();
    this.preChecker = new ToolPreChecker(logger);
  }
  ...
  async addCustomTool(c: Context<AppContext>): Promise<Response> {
    try {
      ...
      const { code, message, status } =
        this.errorHandler.handleAddToolError(error);
  ...
  private async handleLegacyFormatAddTool(
    ...
    // 边界条件预检查
    const preCheckResult = this.preChecker.performPreChecks(
      workflow,
      customName,
      customDescription
    );
    ...
    // 转换工作流为工具配置
    const tool = this.cozeWorkflowHandler.convertWorkflowToTool(
      workflow,
      customName,
      customDescription,
      parameterConfig
  ...
  async updateCustomTool(c: Context<AppContext>): Promise<Response> {
    try {
      ...
      const { code, message, status } =
        this.errorHandler.handleUpdateToolError(error);
      return c.fail(code, message, undefined, status);
    }
  }
  ...
  private async handleUpdateCozeTool(
    ...
    // 更新工具的 inputSchema
    const updatedInputSchema = this.cozeWorkflowHandler.generateInputSchema(
      workflow,
      parameterConfig
    );
  ...
  async removeCustomTool(c: Context<AppContext>): Promise<Response> {
    try {
      ...
      const { code, message, status } =
        this.errorHandler.handleRemoveToolError(error);
      return c.fail(code, message, undefined, status);
    }
  }
  ...
  private async handleEnableTool(
    ...
    // 验证服务存在性
    this.validator.validateServiceAndToolExistence(serverName, toolName);
