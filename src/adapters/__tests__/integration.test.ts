...
import { convertLegacyToNew, getConfigTypeDescription } from '../ConfigAdapter.js';
import { MCPTransportType } from '../../services/MCPService.js';
import type { LocalMCPServerConfig, SSEMCPServerConfig } from '../../configManager.js';
...
describe('适配器集成测试', () => {
  ...
  describe('配置类型描述', () => {
    it('应该返回正确的配置描述', () => {
      ...
      expect(getConfigTypeDescription(sseConfig)).toBe('SSE (https://example.com/sse)');

      const modelScopeConfig: SSEMCPServerConfig = {
        type: 'sse',
        url: 'https://modelscope.net/api/sse'
      };
      expect(getConfigTypeDescription(modelScopeConfig)).toBe('SSE (ModelScope) (https://modelscope.net/api/sse)');
    });
  });
  ...
  describe('工具前缀机制验证', () => {
    it('应该生成正确的工具前缀格式', () => {
      // 这里我们测试前缀格式的一致性
      const testCases = [
        { serviceName: 'calculator', toolName: 'add', expected: 'calculator_xzcli_add' },
        { serviceName: 'weather-service', toolName: 'get_weather', expected: 'weather_service_xzcli_get_weather' },
        { serviceName: 'my-tool', toolName: 'process', expected: 'my_tool_xzcli_process' }
      ];
  ...
  describe('错误处理', () => {
    it('应该在无效配置时抛出错误', () => {
      expect(() => convertLegacyToNew('', { command: 'test' }))
        .toThrow('服务名称必须是非空字符串');

      expect(() => convertLegacyToNew('test', null as any))
        .toThrow('配置对象不能为空');

      expect(() => convertLegacyToNew('test', { invalid: 'config' } as any))
        .toThrow('无法识别的配置类型');
    });
  });
});
