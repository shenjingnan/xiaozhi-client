...
import { configManager, MCPServerConfig } from './configManager.js';
...
class MCPClient {
    ...
    async initialize() {
        try {
            ...
            logger.info(`${this.name} initialized with capabilities: ${JSON.stringify((initResult as any).capabilities)}`);
