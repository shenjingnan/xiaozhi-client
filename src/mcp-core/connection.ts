...
export class MCPConnection {
  ...
  private async attemptConnection(): Promise<void> {
    ...
    logger.debug(`[MCP-${this.name}] 正在连接 MCP 服务: ${this.name}`);
  ...
  private async reconnect(): Promise<void> {
    ...
    logger.debug(`[MCP-${this.name}] 检测到会话过期，正在重新连接...`);
  ...
  private async startHeartbeat(): Promise<void> {
    ...
    logger.debug(`[MCP-${this.name}] 心跳检测已启动，间隔: ${interval}ms`);
  }
  ...
  async callTool(
    ...
    try {
      ...
      if (this.isSessionExpiredError(error)) {
        logger.warn(`[MCP-${this.name}] 检测到会话过期，尝试重新连接并重试...`);
