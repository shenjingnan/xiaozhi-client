...
describe("XiaozhiConnectionManagerSingleton", () => {
  ...
  test("应该创建 IndependentXiaozhiConnectionManager 实例", async () => {
    ...
    // 验证实例类型
    expect(instance.constructor.name).toBe("IndependentXiaozhiConnectionManager");
  });
  ...
  test("应该兼容旧配置选项", async () => {
    ...
    // 验证警告消息
    expect(warnMessages.some(msg => msg.includes("废弃的配置选项"))).toBe(true);
    expect(warnMessages.some(msg => msg.includes("loadBalanceStrategy"))).toBe(true);
  });
  ...
  test("应该正确处理重复初始化", async () => {
    ...
    expect(instance1).toBe(instance2);
    expect(logMessages.some(msg => msg.includes("单例初始化成功"))).toBe(true);
  });
  ...
  test("应该能获取当前实例", async () => {
    const instance = await XiaozhiConnectionManagerSingleton.getInstance();
    const currentInstance = XiaozhiConnectionManagerSingleton.getCurrentInstance();
  ...
  test("应该能从失败状态恢复", async () => {
    ...
    expect(instance1).not.toBe(instance2);
    expect(XiaozhiConnectionManagerSingleton.getStatus().state).toBe("initialized");
  });
});