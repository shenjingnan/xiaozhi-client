...
describe("IndependentXiaozhiConnectionManager", () => {
  ...
  describe("实例化和配置", () => {
    ...
    it("应该使用默认配置创建实例", () => {
      ...
      expect(defaultManager).toBeInstanceOf(IndependentXiaozhiConnectionManager);
    });
  ...
  describe("端点管理", () => {
    ...
    it("应该能够获取所有端点", () => {
      ...
      endpoints.forEach(endpoint => (manager as any).addEndpointSimple(endpoint));
  ...
  describe("连接管理", () => {
    ...
    it("应该能够发送消息到指定端点", async () => {
      ...
      await expect(manager.sendMessage(endpoint, message)).resolves.not.toThrow();
    });
    ...
    it("应该在发送消息到未连接端点时抛出错误", async () => {
      ...
      const message = { type: "test", data: "hello" };
      await expect(manager.sendMessage(endpoint, message)).rejects.toThrow("端点未连接");
    });
  });
  ...
  describe("独立管理每个端点的连接", () => {
    it("应该为每个端点创建独立的连接", async () => {
      ...
      endpoints.forEach(endpoint => manager.addEndpoint(endpoint));
      
      // 模拟连接创建
      const createConnectionSpy = vi.spyOn(manager as any, 'createConnection')
        .mockResolvedValue({
          send: vi.fn(),
          readyState: 1,
        });
    ...
    it("应该能够独立断开特定端点的连接", async () => {
      ...
      // 检查第一个端点的连接状态
      const firstStatus = manager.getConnectionStatus().find(s => s.endpoint === endpoints[0]);
      expect(firstStatus?.connected).toBe(false);
      
      // 检查第二个端点是否仍然连接
      const secondStatus = manager.getConnectionStatus().find(s => s.endpoint === endpoints[1]);
  ...
  describe("正确处理端点失败状态", () => {
    it("应该正确处理连接失败的情况", async () => {
      ...
      // 模拟连接失败
      vi.spyOn(manager as any, 'createConnection')
        .mockRejectedValue(new Error("连接超时"));
  ...
  describe("验证无负载均衡行为", () => {
    ...
    it("应该直接访问指定端点，不进行路由选择", async () => {
      ...
      expect(targetConnection?.send).toHaveBeenCalledWith(JSON.stringify(message));
    ...
    it("应该保持每个端点的独立性", async () => {
      ...
      // 向不同端点发送不同的消息
      await manager.sendMessage(endpoints[0], { type: "test1", data: "message1" });
      await manager.sendMessage(endpoints[1], { type: "test2", data: "message2" });
      
      // 验证每个端点接收到自己的消息
      const firstConnection = connectionMap.get(endpoints[0]);
      const secondConnection = connectionMap.get(endpoints[1]);
      
      expect(firstConnection?.lastMessage).toEqual({ type: "test1", data: "message1" });
      expect(secondConnection?.lastMessage).toEqual({ type: "test2", data: "message2" });
      
      // 验证消息没有交叉
      expect(firstConnection?.lastMessage).not.toEqual(secondConnection?.lastMessage);
    });
  });
  ...
  describe("资源清理", () => {
    it("应该能够清理所有资源", async () => {
      ...
      // 验证所有连接都被关闭
      Object.values(mockConnections).forEach((connection: any) => {
  ...
  describe("错误处理", () => {
    ...
    it("应该正确处理发送消息到不存在的端点", async () => {
      ...
      await expect(manager.sendMessage(endpoint, message))
  ...
  describe("配置验证", () => {
    ...
    it("应该接受部分配置", () => {
      ...
      expect(partialConfigManager).toBeInstanceOf(IndependentXiaozhiConnectionManager);
    });
  });
});