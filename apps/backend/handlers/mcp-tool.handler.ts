...
// 从拆分的模块导入 Coze 工作流相关功能
import {
  createHttpHandler,
  formatCozeValidationError,
  generateInputSchema,
  generateToolDescription,
  isCozeValidationError,
  resolveToolNameConflict,
  sanitizeToolName,
  validateCozeWorkflowData,
  validateCozeWorkflowUpdateData,
} from "./coze-workflow.handler.js";

// 从工具验证模块导入共享验证功能
import { validateGeneratedTool } from "@/utils/tool-validation.js";
...
export class MCPToolHandler {
  ...
  private async handleUpdateCozeTool(
    ...
    // 验证工作流数据完整性
    validateCozeWorkflowUpdateData(workflow);

    // 更新工具的 inputSchema
    const updatedInputSchema = generateInputSchema(workflow, parameterConfig);
  ...
  private convertWorkflowToTool(
    ...
    // 验证工作流数据完整性
    validateCozeWorkflowData(workflow);

    // 生成工具名称（处理冲突）
    const baseName = customName || sanitizeToolName(workflow.workflow_name);
    const toolName = resolveToolNameConflict(baseName);
  ...
  private handleAddToolError(error: unknown): {
    ...
    // 数据验证错误 (400)
    if (isCozeValidationError(errorMessage)) {
      return {
        code: "VALIDATION_ERROR",
        message: formatCozeValidationError(errorMessage),
        status: 400,
      };
    }
