...
import { CozeToolService } from "@/services/coze-tool.service.js";
...
export class MCPToolHandler {
  ...
  private ajv: Ajv;
  private cozeToolService: CozeToolService;

  constructor() {
    this.logger = logger;
    this.ajv = new Ajv({ allErrors: true, verbose: true });
    this.cozeToolService = new CozeToolService();
  }
  ...
  private async handleLegacyFormatAddTool(
    ...
    // 使用 CozeToolService 转换工作流为工具配置
    const tool = this.cozeToolService.convertWorkflowToTool(workflow, {
      customName,
      customDescription,
      parameterConfig,
    });
  ...
  private async handleUpdateCozeTool(
    ...
    // 使用 CozeToolService 验证工作流数据完整性
    this.cozeToolService.validateWorkflowUpdateData(workflow);

    // 使用 CozeToolService 生成更新后的 inputSchema
    const updatedInputSchema = this.cozeToolService.generateInputSchema(
      workflow,
      parameterConfig
    );
