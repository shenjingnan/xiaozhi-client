...
interface LegacyAddCustomToolRequest {
  workflow: CozeWorkflowData["workflow"];
  customName?: string;
...
export class MCPToolHandler {
  ...
  /**
   * 处理旧格式的添加工具请求（向后兼容）
   * 旧格式只支持 Coze 工作流，委托给 handleAddCozeTool
   */
  private async handleLegacyFormatAddTool(
    ...
    // 转换为新格式并委托给 handleAddCozeTool
    const cozeData: CozeWorkflowData = {
      workflow,
      customName,
      customDescription,
      parameterConfig,
    ...
    return this.handleAddCozeTool(c, cozeData);
  }
  ...
  private async handleAddCozeTool(
    ...
    // 委托给 CozeHandler 处理
    return this.cozeHandler.addTool(c);
  }
  ...
  private async handleUpdateCozeTool(
    ...
    // 委托给 CozeHandler 处理
    return this.cozeHandler.updateTool(c);
  }
