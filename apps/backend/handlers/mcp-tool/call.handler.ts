...
export class CallHandler {
  ...
  async callTool(c: Context<AppContext>): Promise<Response> {
    try {
      ...
      // 验证服务和工具是否存在
      await this.validateServiceAndTool(
        serviceManager,
        serviceName,
        toolName,
        c.get("logger")
    ...
    if (serviceName === "customMCP") {
      ...
      try {
        ...
        if (targetTool && !targetTool.description) {
          log.warn(`customMCP 工具 '${toolName}' 缺少描述信息`);
        }

        if (targetTool && !targetTool.inputSchema) {
          log.warn(`customMCP 工具 '${toolName}' 缺少输入参数定义`);
        }
      } catch (error) {
        log.error(`验证 customMCP 工具 '${toolName}' 配置时出错:`, error);
        throw MCPError.validationError(
  ...
  private async validateCustomMCPArguments(
    ...
    try {
      ...
      if (!valid) {
        ...
        log.error(`customMCP 工具 '${toolName}' 参数验证失败:`, errorMessage);
      ...
      log.debug(`customMCP 工具 '${toolName}' 参数验证通过`);
    } catch (error) {
      if (error instanceof Error && error.message.includes("参数验证失败")) {
        throw error;
      }

      log.error(`验证 customMCP 工具 '${toolName}' 参数时出错:`, error);
      throw MCPError.validationError(
        MCPErrorCode.TOOL_VALIDATION_FAILED,
