 ...
 * 验证工作流数据完整性
 */
export function validateWorkflowData(workflow: CozeWorkflow): void {
  if (!workflow) {
  ...
  // 验证必需字段
  validateRequiredFields(workflow);
 ...
 * 验证必需字段
 */
function validateRequiredFields(workflow: CozeWorkflow): void {
 ...
 * 验证扣子API配置
 */
export function validateCozeApiConfig(): void {
  // 检查是否配置了扣子token
 ...
 * 验证工具标识符
 */
export function validateToolIdentifier(
  serverName: string,
  toolName: string
): void {
  if (
 ...
 * 验证服务和工具是否存在
 */
export async function validateServiceAndToolExistence(
  serverName: string,
  toolName: string
): Promise<void> {
  // 检查服务是否存在
