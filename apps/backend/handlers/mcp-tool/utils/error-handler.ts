...
import { formatValidationError, isValidationError } from "./pre-check.js";

/**
 * 处理添加工具时的错误
 */
export function handleAddToolError(error: unknown): {
  code: string;
  message: string;
  status: number;
} {
  ...
  // 工具类型错误 (400)
  if (errorMessage.includes("工具类型") || errorMessage.includes("TOOL_TYPE")) {
    return {
  ...
  if (isValidationError(errorMessage)) {
    return {
      ...
      message: formatValidationError(errorMessage),
      status: 400,
    };
  }
 ...
 * 处理删除工具时的错误
 */
export function handleRemoveToolError(error: unknown): {
  code: string;
  message: string;
  status: number;
} {
 ...
 * 处理更新工具时的错误
 */
export function handleUpdateToolError(error: unknown): {
  code: string;
  message: string;
  status: number;
} {
