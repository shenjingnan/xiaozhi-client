...
export class ConfigClient extends BaseClient {
	 ...
	 * 获取 MCP 服务配置
	 */
	async getMcpServers(): Promise<Record<string, unknown>> {
		const response: ApiResponse<{ servers: Record<string, unknown> }> =
	 ...
	 * 获取连接配置
	 */
	async getConnectionConfig(): Promise<unknown> {
		const response: ApiResponse<{ connection: unknown }> = await this.request(
