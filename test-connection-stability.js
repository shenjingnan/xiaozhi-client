...
class ConnectionTest {
  ...
  async connect() {
    return new Promise((resolve, reject) => {
      ...
      this.ws.on('close', (code, reason) => {
        this.isConnected = false;
        console.log(`🔚 ${new Date().toISOString()} 连接关闭 (代码: ${code}, 原因: ${reason})`);
        this.reconnectCount++;
      ...
      this.ws.on('error', (error) => {
        console.error(`❌ ${new Date().toISOString()} 连接错误:`, error.message);
      });
  ...
  handleMessage(message) {
    console.log(`📨 ${new Date().toISOString()} 收到:`, message.method || '响应');
    ...
    if (message.method === 'initialize') {
      ...
      this.sendResponse(message.id, {});
      this.pingCount++;
      console.log(`🏓 ${new Date().toISOString()} 回应第${this.pingCount}次ping`);
    }
  }
  ...
  printStatus() {
    const uptime = this.connectionStart ? 
      Math.floor((new Date() - this.connectionStart) / 1000) : 0;
